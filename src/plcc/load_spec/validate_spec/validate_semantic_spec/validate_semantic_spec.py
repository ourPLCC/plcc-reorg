...
class SemanticValidator:
    ...
    def _checkTargetLocatorClassName(self, codeFragment: CodeFragment):
        if not re.match(r'^(\w+)(?::([a-z]+))?\s*(?:#.*)?$', codeFragment.targetLocator.className):
            self.errorList.append(InvalidClassNameError(codeFragment.targetLocator.line,
            f"Invalid name format for ClassName {codeFragment.targetLocator.className}, (Must start with an upper case letter, and may contain upper or lower case letters, numbers, and underscore)."))
