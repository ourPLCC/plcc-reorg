...
def test_one_skip_token_matched():
    ...
    assert lexical_spec.ruleList == [makeLexicalRule(Line('skip WHITESPACE \',\'', 5, None), True, 'WHITESPACE', ',')]

def test_one_token_matches():
    lexical_spec = parse_lexical_spec([makeLine('token MINUS \'\\-\'', 8, None)])
    assert lexical_spec.ruleList == [makeLexicalRule(Line('token MINUS \'\\-\'', 8, None), False, 'MINUS', '\\-')]

def test_one_token_no_token_at_start():
    lexical_spec = parse_lexical_spec([makeLine('MINUS \'\\-\'', 8, None)])
    assert lexical_spec.ruleList == [makeLexicalRule(Line('MINUS \'\\-\'', 8, None), False, 'MINUS', '\\-')]
...
def test_tokens_captured_with_trailing_comment():
    ...
    assert lexical_spec.ruleList == [makeLexicalRule(Line('MINUS \'\\-\'  #This is a test', 8, None), False, 'MINUS', '\\-'), makeLexicalRule(Line('token COMMA \',\' #This is also a test', 9, None), False, 'COMMA', ',')]
...
def test_parser_differentiates_between_Line_and_LexicalRule():
    ...
    assert lexical_spec.ruleList == [Line('This line is even more gibberish', 24, None), makeLexicalRule(Line('token LPAREN \'(\'', 23, None), False, 'LPAREN', '(')]

def test_comments_count_as_token_rule():
    lexical_spec = parse_lexical_spec([makeLine('skip COMMENT \'#\'', 2, None)])
    assert lexical_spec.ruleList == [makeLexicalRule(Line('skip COMMENT \'#\'', 2, None), True, 'COMMENT', '#')]

def test_comments_at_end_ignored_and_no_comment_is_wrong():
    lexical_spec = parse_lexical_spec([makeLine('skip COMMENT \'%\' #Ironic isn\'t it?', 3, None), Line('skip COMMENT \'#\' I forgot the comment', 4, None)])
    assert lexical_spec.ruleList == [makeLexicalRule(Line('skip COMMENT \'%\' #Ironic isn\'t it?', 3, None), True, 'COMMENT', '%'), Line('skip COMMENT \'#\' I forgot the comment', 4, None)]
